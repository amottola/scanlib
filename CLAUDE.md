...
- **`gray_to_bw`** — grayscale to 1-bit packed conversion (configurable threshold, default 128)
...
`_get_backend()` in `__init__.py` selects the backend by `sys.platform` and caches it globally. On Linux and Windows, a `_CompositeBackend` wraps the platform backend together with the eSCL backend — `list_scanners()` runs both in parallel and deduplicates. A platform scanner and an eSCL scanner are treated as the **same physical device** when their `Scanner.uuid` values match **or** their IPs match (the eSCL IP comes from mDNS; the platform IP, when present, is parsed out of the scanner's name, e.g. a SANE `escl:` URI). `Scanner.uuid` is a public field populated on macOS (where it equals `id`), on WIA for network/WSD scanners (extracted from the WSD port name via `_extract_wsd_uuid`), and on eSCL (from the mDNS `UUID` record); it is `None` on SANE. Which duplicate survives is controlled by the composite's `prefer_escl` flag, which `_get_backend()` sets from the `SCANLIB_ESCL=1` environment variable: on **Linux/Windows** the platform driver wins by default (the eSCL duplicate is dropped), because the platform backend is always on, but setting `SCANLIB_ESCL=1` flips the preference so the eSCL entry wins there too. On macOS, only `MacOSBackend` is used by default (ImageCaptureCore already handles eSCL natively); setting `SCANLIB_ESCL=1` is what enables the composite backend there at all, and since enabling it signals a preference for eSCL the macOS composite always runs with `prefer_escl=True`. Because ImageCaptureCore delivers discovery callbacks on the main thread's run loop, the composite pumps the run loop (via `pump_run_loop` in `_macos.py`) while waiting for the discovery threads when called on the macOS main thread — otherwise the platform backend would be starved and return nothing. The UUID match is what collapses the ImageCaptureCore/eSCL duplicate for the same network scanner on macOS, and the WIA/eSCL duplicate on Windows. Each backend handles its own thread safety internally:

- **SANE**: used directly (synchronous ctypes, thread-safe)
- **macOS**: `MacOSBackend` uses a lock and main-thread dispatch — from the main thread, calls run directly; from a background thread, calls are forwarded via `performSelectorOnMainThread:withObject:waitUntilDone:modes:` with `waitUntilDone:NO` and the common run-loop modes (ImageCaptureCore delivers callbacks to the main thread — verified: a browser started on a secondary thread still calls back on the main thread, so a private worker run loop does *not* work). Background-thread usage requires the main thread to be running a run loop; every wait is bounded so that when it isn't, calls raise `MainThreadUnavailableError` instead of hanging. `_call` publishes the caller's deadline/cancel onto the worker thread via the `_dispatch_ctx` thread-local, and `_on_main` reads it back through `_dispatch_budget()` — this is why the ~20 `_on_main` call sites don't each take a timeout. The per-dispatch ceiling is `MAIN_DISPATCH_TIMEOUT` (10 s), floored at 0.25 s so the last dispatch of an operation still gets a chance once the overall budget is spent. Headless macOS callers should set `SCANLIB_ESCL=1`, which routes discovery/scanning through eSCL and needs no run loop.
...
1. `list_scanners()` returns lightweight `Scanner` objects (no device session). `str(scanner)` returns `location` if set, otherwise the backend-computed `display_name` — the label the OS shows for the scanner (`vendor model` on SANE, since its `name` is a device URI; the native device name on macOS/WIA/eSCL). It falls back to `vendor model`/`name` when no `display_name` was supplied (e.g. a scanner opened by id). Each Scanner has `id` (unique per device: device URI on SANE, UUID on macOS, WIA device ID on Windows, `escl:{ip}:{port}` on eSCL), `name` (platform-specific: device URI on SANE, display name on macOS/WIA, `ty` TXT record on eSCL), `vendor` (scanner manufacturer when available: SANE, macOS, and WIA — the WIA value is read from the PnP property store, not WIA itself; None on eSCL), `model` (SANE and WIA — likewise from the PnP store on WIA; None on macOS/eSCL), `location` (free-form string from the mDNS `note`; on macOS only the bus descriptor — e.g. "USB" — for locally attached devices, since ImageCaptureCore's `locationDescription` carries the *Bonjour name* for network scanners, which `_name_and_location` routes to `display_name` instead of `location`), and `uuid` (lower-cased device UUID used for cross-backend dedup: the device UUID on macOS, the WSD UUID on WIA network scanners, the mDNS `UUID` on eSCL; None on SANE).
...
Manufacturer and model are not exposed by WIA itself (its `WIA_DIP_VEND_DESC` reports the *driver* vendor — e.g. "Microsoft" for the generic WSD driver — and there is no model property). `_read_pnp_vendor_model` reads the real `DEVPKEY_Device_Manufacturer`/`DEVPKEY_Device_Model` from the Windows PnP property store via `cfgmgr32`: the device interface path from `WIA_DIP_PNP_ID` is resolved to a device instance id with `CM_Get_Device_Interface_PropertyW` (which handles the path→id separator quirks), then to a devnode via `CM_Locate_DevNodeW`, then read with `CM_Get_DevNode_PropertyW`. Any failure degrades silently to `(None, None)`.
...
Built-in multicast DNS client using only `socket` and `struct`. Sends PTR queries for `_uscan._tcp.local.` and `_uscans._tcp.local.` to `224.0.0.251:5353` and parses responses for PTR, TXT, SRV, A, and AAAA records. Because the client binds the multicast port and joins the group, it also receives unsolicited announcements for unrelated services on the LAN (screen sharing on `_rfb._tcp`, AirPlay, `_companion-link`, …); `_parse_responses` therefore keeps only PTR records whose owner name is one of the queried `_uscan`/`_uscans` service types (`_SERVICE_TYPE_SET`), so other devices (iPhones, Macs, VMs) are not misreported as scanners. The query is **retransmitted** with growing intervals (capped at 1s) until a service resolves or the timeout elapses — multicast is lossy (especially over Wi-Fi) and a single query/response can be dropped or suppressed as a duplicate, which previously made discovery flaky on macOS (where the always-running `mDNSResponder` triggers duplicate suppression). A browse is only considered "answered" once a service instance resolves to an IP (`_resolvable_instances`); `_resolve_srv_addresses` links A/AAAA records carried under the SRV target hostname to the service instance name, including across separate packets.
